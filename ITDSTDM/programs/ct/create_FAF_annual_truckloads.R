...
create_FAF_annual_truckloads <- function(faf_filename) {
    ...
    FN <- file.path(RTP[["ct.properties.folder"]], "faf35-truck-allocation-factors.csv")
    ...
    FN <- file.path(RTP[["ct.properties.folder"]], "faf35-truck-equivalency-factors.csv")
    raw <- fread(FN)
    ...
    FN <- file.path(RTP[["ct.properties.folder"]], "faf35-empty-truck-factors.csv")
    raw <- fread(FN)
    ...
    # Save the results to CSV if desired for later processing
    print(str_c(nrow(combined), " FAF truckload equivalents generated"), quote=FALSE)
    if (RTP[["ct.extended.trace"]]=="TRUE") {
        FN <- file.path(RTP[["ct.working.folder"]], "faf35-annual-trucks.csv")
        write.table(combined, file=FN, sep=',', quote=FALSE, row.names=FALSE)
        print(str_c(nrow(combined), " records saved to ", FN), quote=FALSE)
    }

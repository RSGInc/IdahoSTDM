#//////////////////////////////////////////////////////////////////////////////
#////                                                                       ///
#//// Copyright RSG, 2019-2024.                                             ///
#//// Rights to use and modify are granted to the                           ///
#//// Idaho Department of Transportation and partner agencies.              ///
#//// This copyright notice must be preserved.                              ///
#////                                                                       ///
#////                                                                       ///
#//////////////////////////////////////////////////////////////////////////////
#
# Reviews all inputs to the Idaho Statewide Travel Demand Model for possible issues that will result in model errors
# 
#
# Files referenced:
...
class input_checker():
	...
	def checks(self):
		...
		self.inputs_checks = pd.read_csv(self.inputs_checks_path, comment = '#')

		# remove all commented checks from the checks list, remove those not in test group
		self.inputs_checks = self.inputs_checks.loc[[not i for i in (self.inputs_checks['Test'].str.startswith('#'))]]
		self.inputs_checks = self.inputs_checks.loc[self.inputs_checks.Test_Group == self.group]
	...
	def prop_file_paths(self):
		...
		prop_in = pd.read_csv(_join(os.environ["INPUT_FOLDER"], 'pt.properties'), comment = '#', sep = '=', names = ['key', 'value'])
		props = dict((v['key'].strip(), v['value'].strip()) for i, v in prop_in.iterrows())
		
		for item, row in prop_files.iterrows():
			input_table = row['Input_Table']
			input_path = props[row['Property_Token']]
			self.prop_input_paths[input_table] = input_path.replace('%OUTPUT_FOLDER%', os.environ["OUTPUT_FOLDER"]).replace('%INPUT_FOLDER%', os.environ["INPUT_FOLDER"])
	...
	def write_log(self):
		...
		checks_df = pd.merge(self.inputs_checks, self.inputs_list, on='Input_Table', how = 'left')
		...
		def write_check_log(self, fh, row):
			...
			#fh.write("\r\n\t Input File Name: " + ('NA' if not pd.isnull(row['Emme_Object']) else (self.prop_input_paths[row['Input_Table']].rsplit('/', 1)[-1])))
			#fh.write("\r\n\t Input File Location: " + ('NA' if not pd.isnull(row['Emme_Object']) else (_join(self.input_checker_path, self.prop_input_paths[row['Input_Table']].replace('/','\\')))))

...
BY = %BASE_YEAR% 
 ...
 ;FIX (External TAZs that do not have demand produced or attracted):
 COMP dummy = '4344,5586,5600,5621,5631,5640,5654-5659,5661-5665,5668,5670,5671';dummy external tazs
...
;   ;NEED TO UPDATE THESE LINK IDs FOR  THE 35 STATIONS:
;   PHASE=ILOOP
...
;     ;NOT SURE WHAT THIS DOES - refer to "dummy" above
;     IF(!(I = @dummy@))
;       JLOOP
;         IF(!(J = @dummy@))
;           MW[1] = 0
...
;    ;FIX THE REFERENCES TO THE OLD TAZ IDs:
;    JLOOP 
...
RUN PGM=NETWORK
  ...
  NODEI[1]="%INPUT_FOLDER%\externals-new.csv",VAR=N,Vehicles,GrowthRate,VISTPCT,Name
  ...
  PHASE=INPUT FILEI=NI.1
    ...
    IF ((N>=@FEXT@ & N<=@LEXT@))
      ...
      _FORECASTCNT[_i] = (_VEH[_i]*_VISTPCT[_i])*(1+(_GWR[_i]/100))^((@YEAR@)-(@BY@))

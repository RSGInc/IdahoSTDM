...
RUN PGM=NETWORK
  ...
  nodei[1]="%INPUT_FOLDER%/itdnew.net"
...
RUN PGM=NETWORK
	...
	linki[1]="%INPUT_FOLDER%/itdnew.net"
	nodei[2]="%OUTPUT_FOLDER%/node_area_type.txt",VAR=N, AT
	neto="%OUTPUT_FOLDER%/itdnew.net"

	PHASE=NODEMERGE
		AT=NI.2.AT
	ENDPHASE
	;set link area type to max of node area types
	;THIS DOESN'T SEEM TO WORK BECAUSE A AND B ARE IN THE LINK LAYER BUT AT IS IN THE NODE LAYER
	phase=linkmerge
		;M(659): There are no file variables or Links
		IF(A.AT<=B.AT)
			AT=B.AT
		ENDIF
		IF(A.AT>B.AT)

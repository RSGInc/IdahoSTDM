...
RUN PGM=NETWORK
  ...
  ;2829 records read from NODEI[1]: inputs2020\taz_area.csv
  ;1 records contained errors
  ;1 field errors
  nodei[1]="%INPUT_FOLDER%\taz_area.csv" VAR=STDM_TAZ,Area,RENAME=STDM_TAZ-N
  linki[1]="%OUTPUT_FOLDER%\itdnew.net"
  ...
  phase=linkmerge  
    ;M(664): Unconnected Zones:  2829-3000
    IF(A<=%NZONES%)     
...
RUN PGM=NETWORK
  ...
  ;These csvs also don't seem to be read in correctly   
  ...
  PHASE=NODEMERGE 
  ;This N reference doesn't seem to be working, needs to point to a specific input value?
    IF(N <@GAPEND@)
  ...
  ENDPHASE
  ;W(658): PRINT (#1) has undefined LIST fields which will be printed as ?:        
